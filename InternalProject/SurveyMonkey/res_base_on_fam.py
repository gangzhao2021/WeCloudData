...
class Response_Base_On_Family:
    ...
    def single_choice_response(self, answers, response_dict, response_path):
    ...
    def matrix_response(self, answers, response_dict, response_path):
        for answer in answers:
    ...
    def opend_ended_response(self, answers, response_dict, response_path):

        self.text = unicodedata.normalize("NFKD", answers[0]["text"])
        response_dict['text'] = self.text
        self.append_json_to_file(response_path, response_dict)

    def demographic_response(self, answers, response_dict, response_path):
        for answer in answers:
            self.roid = answer["row_id"]
    ...
    def datetime_response(self, answers, response_dict, response_path):
        for answer in answers:
            self.roid = answer["row_id"]
    ...
    def multiple_choice_response(self, answers, response_dict, response_path):
        for answer in answers:
            if "other_id" in answer.keys():
                self.oid = answer["other_id"]
