...
class Retrieve_Responses_Info:
    ...
    def retrieve_response_info(self, survey_id: int) -> dict:
        ...
        with open(self.new_responses_data) as f:
                            ...
                            for res_qtn in res_qtns:
                                ...
                                if fam_id == 3:
                                    self.res_fam.opend_ended_response(
                                        answers, res_info, self.open_ended_response_path)
                                    self.res_fam.opend_ended_response(
                                        answers, res_info, self.new_open_ended_response_path)
                                if fam_id == 1:
                                    self.res_fam.single_choice_response(
                                        answers, res_info, self.single_choice_response_path)
                                    self.res_fam.single_choice_response(
                                        answers, res_info, self.new_single_choice_response_path)
                                if fam_id == 6:
                                    self.res_fam.multiple_choice_response(
                                        answers, res_info, self.multiple_choice_response_path)
                                    self.res_fam.multiple_choice_response(
                                        answers, res_info, self.new_multiple_choice_response_path)
                                if fam_id == 2:
                                    self.res_fam.matrix_response(
                                        answers, res_info, self.matrix_response_path)
                                    self.res_fam.matrix_response(
                                        answers, res_info, self.new_matrix_response_path)
                                if fam_id == 4:
                                    self.res_fam.demographic_response(
                                        answers, res_info, self.demographic_response_path)
                                    self.res_fam.demographic_response(
                                        answers, res_info, self.new_demographic_response_path)
                                if fam_id == 5:
                                    self.res_fam.datetime_response(
                                        answers, res_info, self.datetime_response_path)
                                    self.res_fam.datetime_response(
                                        answers, res_info, self.new_datetime_response_path)
                                if fam_id == 7:
                                    print(
                                        "presentation family found. Please implement a parse function in res_base_on_fam.py")
